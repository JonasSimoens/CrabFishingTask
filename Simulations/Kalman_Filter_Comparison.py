...
def simulation(env, alfa):
    ...
    for trial in range(10):
        ...
        gain = alfa / (alfa + 0.125**2)
        error = crab - cage
        cage = cage + gain * error
        alfa = alfa * (1 - gain)
...
for sim in range(10):
    ...
    for env in range(3):
        ...
        for part in range(50):
            ...
            while alfa <= 0 or alfa >= 1:
                alfa = np.random.normal(alfa_mu, 0.15)
    
            for block in range(20):
                
                frame = simulation(env+1, alfa)
    ...
    looic_comp.to_csv("Kalman_Filter_Comparison_{}.csv".format(sim+1))
